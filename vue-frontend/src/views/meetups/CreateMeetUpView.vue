<template>
  <div class="page">
    ...
    <div v-if="isAuthenticated" class="form">
      ...
      <label class="form-element">
        <span class="form-element-description">Time</span>
      ...
      <label class="form-element">
        <button @click="createMeetUp()">Create</button>
      </label>
    </div>
    <div v-else>You have to be logged in to create a meet up</div>
    <FooterComponent />
  </div>
</template>
...
export default {
  ...
  methods: {
    async createMeetUp() {
      await Api.createMeetup(this.title, this.content, this.time, {
